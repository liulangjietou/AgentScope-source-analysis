...
根 `pom.xml`（`io.agentscope:agentscope-parent`，版本 `<revision>` = 2.0.0-SNAPSHOT）聚合 6 个模块：
...
| `agentscope-extensions` | 全部外部集成，约 50 个子模块：模型厂商、RAG、长期记忆、存储、云沙箱、协议、IM 渠道、调度器、Spring Boot Starter | → core，部分（沙箱/技能类）→ harness |
| `agentscope-examples` | 示例：`documentation`（按文档章节组织的示例类）+ 4 个完整应用（agentscope-builder / codingagent / dataagent / paw） | → core/harness/extensions |
...
flowchart LR
    ...
    DIST --> EXTS["agentscope-extensions<br/>约 50 个子模块"]
...
## 1.2 agentscope-core 的 16 个包地图

包根：`agentscope-core/src/main/java/io/agentscope/core/`。顶层只有两个类：`ReActAgent.java`（4491 行，框架心脏，第 3 章专讲）与 `Version.java`。
...
flowchart TB
    subgraph CORE_PKG["核心包：任何一次 call 都会经过"]
        ...
        EVENT["event/<br/>AgentEvent 33 种流式事件"]
...
| `event` | `AgentEvent.java`、`AgentEventType.java`、`AgentEventEmitter.java` | 细粒度流式事件（33 种） | 第 6 章 |
...
| `util` | `JsonUtils.java`、`JsonSchemaUtils.java`、`MessageUtils.java` | JSON 编解码、Schema 生成、消息工具 | 各章穿插 |
...
3. `ReActAgent.java` 的内部类 `CallExecution`（约 1419–3400 行）—— 全部循环逻辑都在这里
4. `tool/Toolkit.java`（1031 行）→ `tool/ToolExecutor.java`
5. `model/ChatModelBase.java` → 挑一个厂商扩展（推荐 dashscope）读完整实现
6. `harness/agent/HarnessAgent.java`（2485 行）—— 重点读 `Builder#build` 的 middleware 装配段
