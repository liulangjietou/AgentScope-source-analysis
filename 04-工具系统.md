...
`Toolkit.java`（1060 行）是门面类，内部职责拆给五个组件；`ToolExecutor` 是包私有的真正执行器：
...
- **工具与分组是 n:m**：`ToolGroupManager` 一直支持一个工具属于多个组，但早期 `Toolkit` 只在注册时能指定组。现在有了 `Toolkit#addToolToGroup(groupName, toolName)`——把**已注册**的工具追加进另一个已存在的组，重复添加幂等，组或工具不存在则 `IllegalArgumentException`（fast-fail）。典型用途：同一个 `query_order` 既属于 `order` 组，也属于 `after_sales` 组，不必注册两遍。
...
- 自定义权限判断覆写的是 `ToolBase#checkPermissions(Map<String,Object>, PermissionContextState)`（`ToolBase.java:196`）。老代码里的 `tool/ToolExecutionContext` 已 `@Deprecated`，它承担的"给工具传上下文"职责由 `RuntimeContext` 接手，权限侧的上下文则统一为 `PermissionContextState`——升级时按这两个方向拆。
...
第 3 章的 `Toolkit#callTools`（`Toolkit.java:511`）→ `ToolExecutor#executeAll`（`ToolExecutor.java:305`）→ 单个工具走 `executeWithInfrastructure`（`:374`，调度/超时/重试/停机守卫）→ `executeCore`（`:184`）：
...
flowchart TB
    ...
    S3 --> S4["4 ToolValidator#validateInput<br/>JSON Schema 入参校验<br/>错误信息带字段路径"]
...
第 4 步的校验失败信息是**给模型看的**，所以格式很关键：每条错误前面会拼上 `getInstanceLocation()`（JSON Pointer 形式的字段路径），多条之间用换行分隔，例如

```
/orderId: string found, integer expected
/pageSize: must have a maximum value of 100
```

早期只输出 `getMessage()` 并用 `; ` 拼接，模型看到"must be integer"却不知道是哪个参数，往往原样重试同一个错误入参。**把定位信息交给模型，它才有可能自己改对**——这是"工具错误软着陆"能真正闭环的前提。
...
**给 MCP 请求带业务元数据**：`mcp/McpMeta` 是一个类型命名空间标记——把它的实例注册进 `RuntimeContext` 的类型化属性，其中的条目会自动填进每次 `CallToolRequest` 的 `meta` 字段。多租户场景下往 MCP server 透传租户 id、trace id 就靠它。设计上有一道刻意的闸门：**只有注册在 `McpMeta.class` 这个 key 下的对象会被提取**，`RuntimeContext` 里的其它东西（DI bean、配置对象、业务 POJO）绝不会泄漏到 MCP meta 里——和 4.2 的"未标注 `@ToolParam` 的参数按类型注入"是同一套 `RuntimeContext` 机制，但边界收得更紧，因为这些数据要出进程。
...
`agentscope-core/pom.xml:121` 声明了 `io.github.bonede:tree-sitter` + `tree-sitter-bash`，注释写明用途是"Bash 工具的 AST-based safety analysis"，但**截至 2.0.3-SNAPSHOT 主干，仓库内仍没有任何 Java 代码引用它**（无 import、无 `TSParser`/`TSNode` 等 API 调用、无反射加载），`UnixCommandValidator` 只 import 了 `java.util.Set` 与 slf4j。这是个为将来升级 AST 版校验器预留的依赖。
